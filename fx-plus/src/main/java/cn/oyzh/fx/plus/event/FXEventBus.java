...
public class FXEventBus extends EventBus {
    ...
    public <C extends EventConfig> void post(Object event, C config, Integer delayMillis) {
        if (config instanceof FXEventConfig fxConfig&&fxConfig.isFxThread()) {
            ...
            if (delay && async && fxThread) {
                FXUtil.runLater(func, delayMillis);
                // TaskManager.startDelay(() -> FXUtil.runLater(func,delayMillis), delayMillis);
