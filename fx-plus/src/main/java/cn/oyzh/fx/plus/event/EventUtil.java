...
public class EventUtil {
     ...
     * @param event 事件
     */
    public static void post(Event<?> event, boolean async, boolean fxThread, boolean verbose) {
        post(event, EventConfigBuilder.newBuilder().async(async).verbose(verbose).fxThread(fxThread).build());
    }
    ...
    public static void post(Event<?> event, EventConfig config) {
        ...
        Runnable func = () -> {
            ...
            if (config.isVerbose()) {
                startTime = System.currentTimeMillis();
                StaticLog.debug("post event[type={}] start.", event.type(), event.getClass());
            }
            eventBus().post(event);
            // 打印日志
            if (config.isVerbose()) {
        ...
        } else if (config.isDelay() && config.isAsync()) {
            TaskManager.startDelay(func, config.getDelay());
        } else if (config.isAsync() && config.isFxThread()) {
