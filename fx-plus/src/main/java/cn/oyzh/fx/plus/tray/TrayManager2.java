 ...
 * @since 2023/12/21
 */
public class TrayManager2 {

    /**
     * 系统托盘对象，一般程序生命周期内就存在一个
     */
    private static Tray2 tray;
     ...
     * @return 托盘
     */
    public static Tray2 init(String icon) {
        try {
            if (tray == null) {
                ...
                synchronized (TrayManager2.class) {
                    TrayManager2.tray = new Tray2(icon);
                }
            }
            return tray;
        } catch (Exception ex) {
     ...
     * @param trayItem 托盘菜单
     */
    public static void addMenuItem(TrayItem2 trayItem) {
        if (tray != null) {
