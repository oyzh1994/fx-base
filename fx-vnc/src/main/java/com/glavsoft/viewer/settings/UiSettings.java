...
public class UiSettings {
    ...
    public void copyDataFrom(UiSettingsData other, int mask) {
        ...
        if ((mask & CHANGED_MOUSE_CURSOR_SHAPE) == 0) {
            uiSettingsData.setMouseCursorShape(other.getMouseCursorShape());
        }
        if ((mask & CHANGED_FULL_SCREEN) == 0) {
            uiSettingsData.setFullScreen(other.isFullScreen());
        }
    }
