...
public class RdpClient {
    ...
    private BiConsumer<String, Throwable> onDisconnected;
    ...
    private class EmbeddedContext implements IContext {
        ...
        private final com.tangluobo.rdp4j.State attemptState;
        private volatile RdesktopCanvas attemptCanvas;
        private volatile boolean loggedOn = false;
        private volatile boolean ready = false;

        EmbeddedContext(long attemptId, com.tangluobo.rdp4j.State attemptState) {
        ...
        public void error(Exception e, boolean sysexit) {
            ...
            if (sysexit) {
                notifyDisconnected("连接错误: " + describeException(e), e);
            }
        }
        ...
        public void screenResized(int width, int height, boolean clientInitiated) {
            JulLog.info("屏幕大小变更: " + width + "x" + height);
        }
...
//    }
    ...
    public void connect(String host, int port, String username, String password,
        ...
        com.tangluobo.rdp4j.RdpTlsFix.apply(permissiveTrustManager);
        ...
        VChannels channels = new com.tangluobo.rdp4j.FixedVChannels(state);
        ...
        // 因缺少SNI扩展而重置TLS连接
        com.tangluobo.rdp4j.RdpTlsFix.injectRdpTransport(rdpLayer, host);
        ...
        try {
            ...
            notifyDisconnected("服务器已关闭连接", null);
            ...
            notifyDisconnected(describeDisconnect(e), e);
        } catch (RdesktopLicenseException e) {
            JulLog.error("RDP许可证错误: " + e.getMessage());
            notifyDisconnected("许可证错误: " + e.getMessage(), e);
            ...
            notifyDisconnected("连接异常: " + e.getMessage(), e);
        } catch (java.net.UnknownHostException e) {
            JulLog.error("无法解析主机: " + host);
            notifyDisconnected("无法解析主机: " + host, e);
        } catch (java.net.ConnectException e) {
            JulLog.error("连接被拒绝: " + e.getMessage());
            notifyDisconnected("连接被拒绝: " + host + ":" + port + " - " + e.getMessage(), e);
        } catch (java.net.SocketException e) {
            if (connected) {
                JulLog.error("连接中断: " + e.getMessage());
                notifyDisconnected("连接中断: " + e.getMessage(), e);
            }
            ...
            notifyDisconnected("IO错误: " + detail, e);
        } catch (Exception e) {
            JulLog.error("RDP连接错误: " + e.getMessage(), e);
            notifyDisconnected("连接错误: " + e.getMessage(), e);
        } finally {
            connected = false;
        }
        //        }, "RDP-" + host);
            ...
            diagTimer.scheduleAtFixedRate(new java.util.TimerTask() {
                ...
                public void run() {
                    ...
                    if (rdpLayer instanceof RdpPatch patch) {
                        JulLog.info(String.format("[DIAG #%d] totalPDUs=%d, bitmapUpdates=%d, rdp5Packets=%d, sent=%d, recv=%d, connected=%b, active=%b, rdp5=%b, licenceIssued=%b, securityType=%s",
                                count[0], patch.getTotalPduCount(), patch.getBitmapUpdateCount(), patch.getRdp5PacketCount(),
                                com.tangluobo.rdp4j.RdpTlsFix.RdpTransport.getSendPktCount(), com.tangluobo.rdp4j.RdpTlsFix.RdpTransport.getRecvPktCount(),
     ...
     */
    private void registerClipboardChannel(com.tangluobo.rdp4j.State state, RdesktopCanvas canvas, VChannels channels) {
    ...
    private void registerRdpdrChannel(VChannels channels) {
        if (!soundEnabled) {
            JulLog.info("远程音频未启用，跳过设备重定向声明通道(rdpdr)");
            return;
        }
        try {
     ...
     * javardp库本身无rdpsnd实现，由{@link com.tangluobo.rdp4j.RdpsndChannel}完整实现
    ...
    private void registerSoundChannel(VChannels channels) {
        ...
        if (!soundEnabled) {
            JulLog.info("远程音频通道(rdpsnd)未启用");
            return;
        }
        try {
            rdpsndChannel = new com.tangluobo.rdp4j.RdpsndChannel();
    ...
    private void retryWithStandardSecurity(String host, int port, CredentialProvider dcp) {
        try {
            ...
            JulLog.info("回退重连: " + host + ":" + port + " securityType=STANDARD");
            rdpLayer.connect(new DefaultIO(InetAddress.getByName(host), port),
                    dcp, options.getCommand(), options.getDirectory());
            JulLog.info("Standard RDP Security连接成功，进入主循环");
            rdpLayer.mainLoop();
            JulLog.info("RDP主循环正常退出");
            notifyDisconnected("服务器已关闭连接", null);
        } catch (com.tangluobo.rdp4j.RdesktopDisconnectException e) {
            ...
            notifyDisconnected("连接失败（SSL和Standard均不可用）: " + e.getMessage(), e);
        } finally {
            connected = false;
        }
    }
    ...
    private void retryWithSslSecurity(String host, int port, CredentialProvider dcp) {
        try {
            ...
            JulLog.info("回退重连: " + host + ":" + port + " securityType=SSL");
            rdpLayer.connect(new DefaultIO(InetAddress.getByName(host), port),
                    dcp, options.getCommand(), options.getDirectory());

            JulLog.info("SSL/TLS连接成功，进入主循环");
            rdpLayer.mainLoop();
            JulLog.info("RDP主循环正常退出");
            notifyDisconnected("服务器已关闭连接", null);
        } catch (com.tangluobo.rdp4j.RdesktopDisconnectException e) {
            ...
            if (msg != null && msg.contains("SSL negotiation failed")) {
                JulLog.warn("SSL/TLS握手失败，尝试使用HYBRID（CredSSP/NLA）重连...");
                retryWithHybridSecurity(host, port, dcp);
                return;
            }
            JulLog.error("SSL/TLS重连也失败: " + e.getMessage(), e);
            notifyDisconnected("连接失败（Standard和SSL均不可用）: " + e.getMessage(), e);
        } finally {
            connected = false;
        }
    }
     ...
     */
    private void retryWithHybridSecurity(String host, int port, CredentialProvider dcp,
                                         com.tangluobo.rdp4j.RdpRedirectionException initialRedirection) {
        ...
        try {
            com.tangluobo.rdp4j.RdpRedirectionException pendingRedirection = initialRedirection;
            ...
            while (true) {
                if (pendingRedirection == null) {
                    try {
                        JulLog.info("HYBRID (CredSSP/NLA) 连接成功，进入主循环");
                        rdpLayer.mainLoop();
                        JulLog.info("RDP主循环正常退出");
                        notifyDisconnected("服务器已关闭连接", null);
                        return;
                    } catch (com.tangluobo.rdp4j.RdpRedirectionException redirectException) {
            ...
            notifyDisconnected("连接失败（Standard、SSL、HYBRID和RDSTLS均不可用）: " + detail,e);
        } finally {
    ...
    private Exception connectHybridWithTokenFallback(String host, int port,
        ...
        com.tangluobo.rdp4j.CredSspTokenMode primaryMode = Boolean.getBoolean("tomata.rdp.preferSpnego")
                ...
                : com.tangluobo.rdp4j.CredSspTokenMode.RAW_NTLM;
        com.tangluobo.rdp4j.CredSspTokenMode fallbackMode = primaryMode == com.tangluobo.rdp4j.CredSspTokenMode.RAW_NTLM
                ? com.tangluobo.rdp4j.CredSspTokenMode.SPNEGO_NTLM
                : com.tangluobo.rdp4j.CredSspTokenMode.RAW_NTLM;
        try {
    ...
    private void connectRdstlsAttempt(String host, int port,
        ...
        com.tangluobo.rdp4j.DefaultCredentialsProvider redirectedIdentity = new DefaultCredentialsProvider(
     ...
     * 设置断开连接回调
     */
    public void setOnDisconnected(BiConsumer<String, Throwable> callback) {
