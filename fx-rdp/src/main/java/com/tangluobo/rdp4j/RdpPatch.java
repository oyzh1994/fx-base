...
public class RdpPatch extends Rdp {
    ...
    private void processFastPathFragment(int type, int fragmentation, com.tangluobo.rdp4j.Packet source, int next)
            throws RdesktopException, OrderException {
        ...
        fastPathFragmentData = null;
        fastPathFragmentType = -1;
        com.tangluobo.rdp4j.Packet packet = new com.tangluobo.rdp4j.Packet(combined);
    ...
    @Override
    public void rdp5_process(Packet s, boolean e)
            throws RdesktopException, OrderException {
    ...
    public void connect(com.tangluobo.rdp4j.io.IO io, com.tangluobo.rdp4j.CredentialProvider credentialProvider,
        ...
        JulLog.info("[CONNECT] Done, securityType=" + stateRef.getSecurityType()
    ...
    public void mainLoop() throws IOException, RdesktopException {
        JulLog.info("[MAINLOOP] Entering main loop, rdp5=" + stateRef.isRDP5());

        if (receiveMethod == null || processPacketMethod == null) {
            JulLog.warn("[MAINLOOP] 反射方法不可用，使用父类mainLoop");
            super.mainLoop();
            return;
        }
