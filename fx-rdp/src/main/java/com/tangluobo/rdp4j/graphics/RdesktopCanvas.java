...
public class RdesktopCanvas {
    ...
    private int height;
    private com.tangluobo.rdp4j.graphics.RdpCursor hiddenCursor;
    ...
    public com.tangluobo.rdp4j.graphics.RdpCursor createCursor(int nXDst, int nYDst, int nWidth, int nHeight, byte[] andMask, byte[] xorMask, int cache_idx,
                                                               int xorBpp) {
        if (nWidth != lastLoggedCursorWidth || nHeight != lastLoggedCursorHeight
                || xorBpp != lastLoggedCursorBpp) {
            JulLog.info("Remote cursor source: {}x{} @ {}bpp, hotspot={},{}",
    ...
    public void drawGlyph(int mixmode, int x, int y, int cx, int cy, byte[] data, int bgcolor, int fgcolor) {
        ...
        // if(logger.isInfoEnabled()) JulLog.info("glyph
        // \t(\t"+x+",\t"+y+"),(\t"+(x+cx-1)+",\t"+(y+cy-1)+")");
    ...
    public void drawLineVerticalHorizontal(int x1, int y1, int x2, int y2, int color, int opcode) {
        ...
        // if(logger.isInfoEnabled()) JulLog.info("line
        // \t(\t"+x1+",\t"+y1+"),(\t"+x2+",\t"+y2+")");
    }
    ...
    public void drawMemBltOrder(MemBltOrder memblt) {
        ...
        if (JulLog.isDebugEnabled())
            JulLog.debug("MEMBLT x=" + x + " y=" + y + " cx=" + cx + " cy=" + cy + " srcx=" + srcx + " srcy=" + srcy + " opcode="
                    + memblt.getOpcode());
        try {
    ...
    public void drawRectangleOrder(RectangleOrder rect) {
        // if(logger.isInfoEnabled()) JulLog.info("RectangleOrder!");
        ...
        try {
            ...
            switch (triblt.getOpcode()) {
                ...
                case 0xc0: // PSa
                    ...
                    break;
                default:
                    JulLog.warn("Unimplemented Triblt opcode:" + triblt.getOpcode());
                    rop.do_array(ROP2_COPY, backstore, this.width, x, y, cx, cy, bitmap.getBitmapData(), bitmap.getWidth(), srcx, srcy);
            }
        } catch (RdesktopException e) {
        }
    ...
    public void fillRectangle(int x, int y, int cx, int cy, int color) {
        ...
        if (JulLog.isDebugEnabled())
            JulLog.debug("rect \t(\t" + x + ",\t" + y + "),(\t" + (x + cx - 1) + ",\t" + (y + cy - 1) + ")/"
        ...
        switch (brush.getStyle()) {
            case 0: // solid
                ...
                for (i = 0; i < src.length; i++)
                    src[i] = fgcolor;
                if (JulLog.isDebugEnabled())
                    JulLog.debug("x=" + x + " y=" + y + " cx=" + cx + " cy=" + cy + " width=" + this.width + " height=" + this.height
            ...
            case 3: // pattern
                ...
                break;
            default:
                JulLog.warn("Unsupported brush style " + brush.getStyle());
        }
    }
    ...
    // * e) { JulLog.warn("Pointer movement not allowed"); } }
    // */
    // }
    ...
    public void triggerReady(ReadyType type) {
        if (type == ReadyType.INPUT && input != null) {
            ...
            JulLog.info("Graphics ready to be sent");
        }
    }
