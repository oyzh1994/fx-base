...
public class WrappedImage extends JComponent implements Display, Scrollable {
	...
	public int getRGB(int x, int y) {
		if (cm == null)
			...
			if (cm.getRGB(out) != bi.getRGB(x, y))
				JulLog.info("Did not get correct colour value for color (" + Integer.toHexString(pix) + "), got (" + cm.getRGB(out)
						+ ") instead");
			return out;
		}
	}
	...
	public void movePointer(int x, int y) {
		java.awt.EventQueue.invokeLater(() -> {
			...
			try {
				...
				JulLog.debug("Unable to apply server pointer position", e);
			}
		});
	}
	...
	public void update(Graphics g) {
		...
		if (JulLog.isDebugEnabled())
			JulLog.debug("Update " + r + " for " + getDisplayWidth() + "x" + getDisplayHeight());
		int w = r.width;
		int h = r.height;
