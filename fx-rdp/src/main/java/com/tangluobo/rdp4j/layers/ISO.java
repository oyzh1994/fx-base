...
public class ISO implements Layer<MCS> {
	...
	public void connect(IO io) throws IOException, RdesktopException {
		...
		JulLog.info(String.format("Connecting with %s enabled.", state.getOptions().getSecurityTypes()));
		...
		if (state.isRDP5() && neg.getPosition() < neg.capacity()) {
			...
			if (result == 0x02) {
				...
				JulLog.info(String.format("Server supports: %s", supportedTypes));
				...
				if (!supportedTypes.contains(wanted)) {
					...
					JulLog.warn(String.format("Server does not support requested security type %s, downgrading to %s.", wanted, alt));
			...
			if (state.isRDP5()) {
				JulLog.info("Downgrading to RDP4 because no negotiation.");
				state.setRDP5(false);
				state.setServerBpp(8);
			}
			if (state.getSecurityType().ordinal() > SecurityType.STANDARD.ordinal()) {
				JulLog.warn(String.format("Downgrading security type to %s from %s because no negotiation.", SecurityType.STANDARD, state.getSecurityType()));
				state.setSecurityType(SecurityType.STANDARD);
			}
		}
		if (state.getSecurityType().isSSL()) {
			JulLog.info("Protocol requires SSL, instantiating.");
			transport.startSsl();
		}
	}
	...
	void send_connection_request() throws IOException {
		...
		if (routingToken != null && routingToken.length > 0) {
			...
			JulLog.info("Including RDP routing token (" + routingToken.length + " bytes)");
		}
		if (StringUtil.isNotBlank(cookie)) {
			if (JulLog.isDebugEnabled())
				JulLog.debug("Including username");
	...
	private Packet receiveMessageex(int[] type, int[] rdpver) throws IOException, RdesktopException {
		if (JulLog.isDebugEnabled())
			JulLog.debug("ISO.receiveMessage");
		Packet s = null;
		int length, version;
		next_packet: while (true) {
			if (JulLog.isDebugEnabled())
				JulLog.debug("next_packet");
			...
			if ((version & 3) == 0) {
				if (JulLog.isDebugEnabled())
					JulLog.debug("Processing rdp5 packet of " + length);
		...
		if (type[0] == DATA_TRANSFER) {
			if (JulLog.isDebugEnabled())
				JulLog.debug("Data Transfer Packet of " + length);
			s.incrementPosition(1); // eot
			return s;
		}
		if (JulLog.isDebugEnabled())
			JulLog.debug("Message of " + length);
