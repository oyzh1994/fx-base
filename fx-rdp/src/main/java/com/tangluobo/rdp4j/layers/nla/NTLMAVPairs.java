...
public class NTLMAVPairs implements com.tangluobo.rdp4j.layers.nla.PacketPayload {
	//
	...
	private com.tangluobo.rdp4j.layers.nla.NTLMSingleHost singleHost;
	...
	public Packet write() throws IOException {
		...
		for (int avId : new int[] { MSV_AV_NB_DOMAIN_NAME, MSV_AV_NB_COMPUTER_NAME, MSV_AV_DNS_DOMAIN_NAME,
			...
			case MSV_AV_NB_COMPUTER_NAME:
				if (StringUtil.isNotBlank(nbComputerName))
					data = nbComputerName.getBytes(com.tangluobo.rdp4j.layers.nla.NTLMState.UNICODE_ENCODING);
				break;
			case MSV_AV_NB_DOMAIN_NAME:
				if (StringUtil.isNotBlank(nbDomainName))
					data = nbDomainName.getBytes(com.tangluobo.rdp4j.layers.nla.NTLMState.UNICODE_ENCODING);
				break;
			case MSV_AV_DNS_COMPUTER_NAME:
				if (StringUtil.isNotBlank(dnsComputerName))
					data = dnsComputerName.getBytes(com.tangluobo.rdp4j.layers.nla.NTLMState.UNICODE_ENCODING);
				break;
			case MSV_AV_DNS_DOMAIN_NAME:
				if (StringUtil.isNotBlank(dnsDomainName))
					data = dnsDomainName.getBytes(com.tangluobo.rdp4j.layers.nla.NTLMState.UNICODE_ENCODING);
				break;
			case MSV_AV_DNS_TREE_NAME:
				if (StringUtil.isNotBlank(dnsTreeName))
					data = dnsTreeName.getBytes(com.tangluobo.rdp4j.layers.nla.NTLMState.UNICODE_ENCODING);
				break;
			case MSV_AV_FLAGS:
				if (flags > 0) {
			...
			case MSV_AV_TARGET_NAME:
				if (StringUtil.isNotBlank(targetName))
					data = targetName.getBytes(com.tangluobo.rdp4j.layers.nla.NTLMState.UNICODE_ENCODING);
				break;
	...
	public void read(Packet packet) throws IOException {
		while (true) {
			...
			JulLog.debug(String.format("Read %s (%d, %02x)", toName(id), id, id));
			switch (id) {
			case MSV_AV_EOL:
			...
			case MSV_AV_TIMESTAMP:
				...
				timestamp = (timestampFileTime / 10000L) - com.tangluobo.rdp4j.layers.nla.NTLMState.MILLISECONDS_BETWEEN_1970_AND_1601;
				break;
			case MSV_AV_SINGLE_HOST:
				int singleHostLength = packet.getLittleEndian16();
				if (singleHostLength < com.tangluobo.rdp4j.layers.nla.NTLMSingleHost.LENGTH)
				...
				singleHost = new com.tangluobo.rdp4j.layers.nla.NTLMSingleHost();
			...
			default:
				...
				JulLog.warn(String.format("Unknown AvId %d (%02x), skipping %d bytes.", id, id, len));
	...
	public void setTimestamp(long timestamp) {
		...
		this.timestampFileTime = (timestamp + com.tangluobo.rdp4j.layers.nla.NTLMState.MILLISECONDS_BETWEEN_1970_AND_1601) * 10000L;
	}

	void setTimestampFileTime(long timestampFileTime) {
		this.timestampFileTime = timestampFileTime;
		this.timestamp = (timestampFileTime / 10000L) - com.tangluobo.rdp4j.layers.nla.NTLMState.MILLISECONDS_BETWEEN_1970_AND_1601;
	}

	public com.tangluobo.rdp4j.layers.nla.NTLMSingleHost getSingleHost() {
		return singleHost;
	}
