...
public class NLA {
	...
	interface TSCredentialType extends com.tangluobo.rdp4j.layers.nla.BerPayload {
	}
	...
	class TSCredentials implements com.tangluobo.rdp4j.layers.nla.BerPayload {
	...
	public void start() throws IOException, RdesktopCryptoException {
		 ...
		 * use
		 */
		com.tangluobo.rdp4j.layers.nla.NTLMState ntlm = new NTLMState(state);
		...
		byte[] negotiateToken = requestedSpnego
				...
				: negotiateData;
		com.tangluobo.rdp4j.layers.nla.TSRequest req = new com.tangluobo.rdp4j.layers.nla.TSRequest(negotiateToken);
		...
		JulLog.info("Sending NTLM Negotiate, tokenFormat="
				+ (requestedSpnego ? "SPNEGO/NTLM" : "RAW_NTLM"));
		ntlm.dumpFlags();
		transport.sendPacket(new Packet(bos.getArray()));
		
		// MS-NLMP - 2.2.1.2 CHALLENGE_MESSAGE
		com.tangluobo.rdp4j.layers.nla.TSRequest challengeRequest = new com.tangluobo.rdp4j.layers.nla.TSRequest(null);
		...
		JulLog.info("CredSSP server version=" + challengeRequest.getVersion()
				+ ", effectiveVersion=" + peerVersion);
		com.tangluobo.rdp4j.layers.nla.NTLMResponse response = new NTLMResponse(ntlm);
		...
		if (requestedSpnego && !useSpnego) {
			JulLog.info("CredSSP server returned raw NTLM; continuing in RAW_NTLM mode");
		} else if (!requestedSpnego && useSpnego) {
			JulLog.info("CredSSP server selected SPNEGO/NTLM; continuing in SPNEGO mode");
		}
		JulLog.info("Received NTLM Challenge");
		...
		JulLog.info(String.format("NTLM Authenticate flags=0x%08x, mic=%b, tokenLength=%d",
		...
		if (micRequired) {
			...
			JulLog.info("NTLM Challenge未要求MIC");
		}
		...
		if (useSpnego) {
			...
			JulLog.info("Sending NTLM Authenticate with SPNEGO mechListMIC");
		}
		com.tangluobo.rdp4j.layers.nla.TSRequest authenticateRequest = new com.tangluobo.rdp4j.layers.nla.TSRequest(authenticateToken);
		...
		send.encode(bos, true);
		JulLog.info("Sending NTLM Authenticate");
		...
		JulLog.info("Receiving NTLM Authenticate Response");
		// 服务器在此返回一个有明确BER长度的TSRequest。不能读到EOF：
		// TLS连接会继续承载RDP会话，读到EOF会永久阻塞。
		com.tangluobo.rdp4j.layers.nla.TSRequest publicKeyResponse = new com.tangluobo.rdp4j.layers.nla.TSRequest(null);
		...
		if (useSpnego && publicKeyResponse.getNegoData() != null) {
			...
			if (serverMechListMic != null) {
				...
				JulLog.info("Verified SPNEGO server mechListMIC");
			}
		}
		...
		JulLog.info("CredSSP server public-key binding verified");
		...
		JulLog.info("Preparing CredSSP delegated credentials: account="
		...
		com.tangluobo.rdp4j.layers.nla.TSRequest credentialRequest = new TSRequest(null);
		...
		bos = new BerByteArrayOutputStream();
		send.encode(bos, true);
		JulLog.info("Sending CredSSP delegated credentials");
		transport.sendPacket(new Packet(bos.getArray()));
	}
