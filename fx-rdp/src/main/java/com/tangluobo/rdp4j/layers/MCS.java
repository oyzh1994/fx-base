...
public class MCS implements Layer<com.tangluobo.rdp4j.layers.Secure> {
	...
	private com.tangluobo.rdp4j.layers.ISO isoLayer = null;
	private State state;
	private com.tangluobo.rdp4j.layers.Secure secure;
	 ...
	 * @param secure secure
	 */
	public MCS(IContext context, State state, VChannels channels, com.tangluobo.rdp4j.layers.Secure secure) {
		this.secure = secure;
		this.state = state;
	...
	public void connect(IO io, Packet data) throws IOException, RdesktopException {
		if (JulLog.isDebugEnabled())
			JulLog.debug("MCS.connect");
		isoLayer.connect(io);
		this.sendConnectInitial(data);
		this.receiveConnectResponse(data);
		if (JulLog.isDebugEnabled())
			JulLog.debug("connect response received");
		send_edrq();
		send_aurq();
	...
	public Packet receive(int[] channel) throws IOException, RdesktopException, OrderException {
		...
		length = buffer.get8();
		if (JulLog.isDebugEnabled())
			JulLog.debug("Channel ID = " + channel[0] + ", length = " + length);
		if ((length & 0x80) != 0) {
	...
	public void receiveConnectResponse(Packet data) throws IOException, RdesktopException {
		if (JulLog.isDebugEnabled())
			JulLog.debug("MCS.receiveConnectResponse");
		...
		/*
		 * if (length > data.size()) {
		 * JulLog.warn("MCS Datalength exceeds size!"+length);
	...
	public void sendConnectInitial(Packet data) throws IOException, RdesktopException {
		...
		if (JulLog.isDebugEnabled())
			JulLog.debug("MCS.sendConnectInitial");
