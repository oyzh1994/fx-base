...
public class NLA3 {
	...
	private void doSPNEGO(String serverPrinc) throws GSSException, IOException, UnsupportedEncodingException {
		...
		for (int i = 0; i < oids.length; i++) {
			JulLog.info(String.format("GSSAPI supports %s. %s", oids[i], Arrays.asList(manager.getNamesForMech(oids[i]))));
		}
		/*
		...
		while (!context.isEstablished()) {
			...
			// initSecContext
			if (token != null) {
				JulLog.info(String.format("Will send token of size %d from initSecContext", token.length));
			...
			if (!context.isEstablished()) {
				...
				JulLog.info(String.format("Will read input token of size %d for processing by initSecContext", token.length));
