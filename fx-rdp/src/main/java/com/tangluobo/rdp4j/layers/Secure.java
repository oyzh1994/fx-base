...
public class Secure implements Layer<Rdp> {
	...
	public void connect(IO io) throws UnknownHostException, IOException, RdesktopException, SocketException, OrderException {
		...
		JulLog.info(String.format("Requested server bpp is %d", state.getServerBpp()));
		mcsLayer.connect(io, mcs_data);
		this.processMcsData(mcs_data);
		if (state.getSecurityType() == SecurityType.STANDARD) {
			this.establishKey();
		} else {
			JulLog.info("Not sending key as encryption has been disabled.");
		}
	}
	...
	public byte[] decrypt(byte[] data) throws RdesktopCryptoException {
		try {
			...
			if (JulLog.isDebugEnabled())
				JulLog.debug(String.format("Decrypting %d bytes", data.length));
			if (this.dec_count == 4096) {
				...
				JulLog.debug("Packet dec_count=" + dec_count);
				this.dec_count = 0;
			}
	...
	public byte[] decrypt(byte[] data, int length) throws RdesktopCryptoException {
		try {
			byte[] buffer = null;
			if (JulLog.isDebugEnabled())
				JulLog.debug(String.format("Decrypting %d bytes (fixed length)", data.length));
			if (this.dec_count == 4096) {
				decryptKey = this.update(decryptKey, decryptUpdateKey);
				this.rc4_dec.init(Cipher.DECRYPT_MODE, new SecretKeySpec(decryptKey, "RC4"));
				JulLog.debug("Packet dec_count=" + dec_count);
	...
	public void establishKey() throws RdesktopException, IOException {
		...
		buffer.markEnd();
		JulLog.info("Sending client key");
	...
	public void generateInitialKeys() throws RdesktopCryptoException {
		...
		if (sessionKeyEncryptionMethod == Secure.SEC_40BIT_ENCRYPTION) {
			JulLog.info("40 Bit Encryption enabled");
			...
			JulLog.info("56 Bit Encryption enabled");
			...
			macKey = macKey128;
			JulLog.info("128 Bit Encryption enabled");
		} else {
	...
	public void generateRandom() {
		 ...
		 * e){JulLog.warn("No Such Random Algorithm");}
		 */
	}
	...
	private void parseServerSecurityData(Packet data) throws RdesktopException {
		JulLog.debug("Secure.parseCryptInfo");
		...
		JulLog.debug(String.format("Key size %d, encryption level %d", serverSessionKeyEncryptionMethod, encryptionLevel));
		...
		if (encryptionLevel > 1) {
			JulLog.info("Server will be sending encrypted packets");
		}
		if (encryptionLevel > 0) {
			JulLog.info("Client will be sending encrypted packets");
		}
		...
		if (end > data.getEnd()) {
			JulLog.debug("Reached end of crypt info prematurely ");
			return;
		}
		...
		// X.509
		JulLog.debug("Flags = 0x" + Integer.toHexString(flags));
		if ((flags & 1) != 0) {
			JulLog.debug(("We're going for the RDP4-style encryption"));
			...
			if (data.getPosition() == data.getEnd()) {
				...
				JulLog.warn("End not reached!");
			}
		} else {
			int nocerts = data.getLittleEndian32(); // number of certificates
			JulLog.debug(String.format("We're going for the RDP5-style encryption (%d certfificates)", nocerts));
	...
	private void processServerSecurityData(Packet data) throws RdesktopException {
		...
		if (state.getSessionKeyEncryptionMethod() == 0) {
			if (state.getSecurityType() == SecurityType.STANDARD) {
				JulLog.info("Disabling encryption, server is not using it.");
				state.setSecurityType(SecurityType.NONE);
			}
			return;
		}
		// this.client_random = this.generateRandom(SEC_RANDOM_SIZE);
		JulLog.debug("readCert = " + state.isReadCert());
		if (state.isReadCert()) { /*
	...
	public void processMcsData(Packet mcs_data) throws RdesktopException {
		JulLog.debug("Secure.processMcsData");
	...
	public void RSAEncrypt(int length, int modulus_size) throws RdesktopException {
		...
		if (this.sec_crypted_random.length > SEC_MAX_MODULUS_SIZE) {
			JulLog.warn("sec_crypted_random too big!"); /* FIXME */
		}
		this.reverse(this.sec_crypted_random);
	...
	public Packet sendMcsData() {
		JulLog.debug("Secure.sendMcsData");
		...
		if (state.isRDP5() && (channels.num_channels() > 0)) {
			JulLog.debug(("num_channels is " + channels.num_channels()));
			...
			// channels
			for (int i = 0; i < channels.num_channels(); i++) {
				JulLog.debug(("Requesting channel " + channels.channel(i).name()));
	...
	private void processSrvInfo(Packet mcs_data) {
		...
		JulLog.debug(("Server RDP version is " + state.getServerRdpVersion()));
		if (state.isNegotiated()) {
			...
			JulLog.info("Early server capabilities is " + earlyCaps);
		}
	}
