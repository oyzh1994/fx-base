...
public class Rdp implements com.tangluobo.rdp4j.layers.Layer<com.tangluobo.rdp4j.layers.Layer<?>> {
	...
	// as 7 byte
	// US-Ascii
	protected com.tangluobo.rdp4j.layers.Secure secureLayer = null;
	boolean deactivated;
	...
	public Rdp(IContext context, State state, VChannels channels) {
		...
		this.state = state;
		secureLayer = new com.tangluobo.rdp4j.layers.Secure(context, state, channels, this);
	...
	public void connect(IO io, CredentialProvider credentialProvider, String command, String directory)
		...
		if (state.getSecurityType() == SecurityType.STANDARD && !state.getOptions().isPacketEncryption()) {
			JulLog.info("Disabling encryption because packet encryption turned off.");
		...
		try {
			while (true) {
				try {
					...
					JulLog.debug("IO error during receive.", ioe);
	...
	private void processPacket(int[] type, Packet data) throws RdesktopException, IOException {
		...
		case (Rdp.RDP_PDU_DEMAND_ACTIVE):
			...
			if (JulLog.isDebugEnabled())
				JulLog.debug("Rdp.RDP_PDU_DEMAND_ACTIVE");
			...
			// before
			// 1st order
			JulLog.info("Past license negotiation");
			context.ready(ReadyType.DISPLAY);
			state.setActive(true);
			break;
		case (Rdp.RDP_PDU_DEACTIVATE_ALL):
			JulLog.info("Got deactivate all.");
			...
			if (state.getLastReason() > 0)
				...
				JulLog.info("Re-negotiate capabilities");
			}
			this.stream = null; // ty this fix
			break;
		case (Rdp.RDP_PDU_DATA):
			if (JulLog.isDebugEnabled())
				JulLog.debug("Rdp.RDP_PDU_DATA");
		...
		case (Rdp.RDP_PDU_SERVER_REDIRECTION):
			...
			JulLog.info("Server Redirection PDU marked LB_NOREDIRECT; staying on current session");
			break;
		case 0:
	...
	private RdpRedirectionInfo processServerRedirection(Packet data) throws RdesktopException {
		...
		JulLog.info("Received Server Redirection PDU: flags=0x"
	...
	public void rdp5_process(Packet s, boolean encryption, boolean shortform) throws RdesktopException, OrderException {
		if (JulLog.isDebugEnabled())
			JulLog.debug("Processing RDP 5 order");
		int length, count;
		int type;
		int next;
		if (encryption) {
		...
		while (s.getPosition() < s.getEnd()) {
			...
			case 12:
				...
				break;
			default:
				JulLog.warn("Unimplemented RDP5 opcode " + type);
			}
			s.setPosition(next);
		}
	}
	...
	public void sendInput(int time, int message_type, int device_flags, int param1, int param2) {
		...
		try {
			...
			JulLog.error("Failed to send input.", e);
			markConnectionClosed();
	...
	protected void process_cached_pointer_pdu(Packet data) throws RdesktopException {
		if (JulLog.isDebugEnabled())
			JulLog.debug("Rdp.RDP_POINTER_CACHED");
		int cache_idx = data.getLittleEndian16();
		if (JulLog.isDebugEnabled())
			JulLog.debug(String.format("Setting cache cursor %d", cache_idx));
		try {
	...
	protected void process_colour_pointer_pdu(Packet data) throws RdesktopException {
		if (JulLog.isDebugEnabled())
			JulLog.debug("Rdp.RDP_POINTER_COLOR");
	...
	protected void process_colour_pointer_pdu_new(Packet data) throws RdesktopException {
		if (JulLog.isDebugEnabled())
			JulLog.debug("Rdp.RDP_POINTER_POINTER");
		int x = 0, y = 0, width = 0, height = 0, cache_idx = 0, masklen = 0, datalen = 0;
		byte[] mask = null, pixel = null;
		RdpCursor cursor = null;
	...
	protected void processBitmapUpdates(Packet data) throws RdesktopException {
		// JulLog.info("processBitmapUpdates");
		int n_updates = 0;
		...
		for (int i = 0; i < n_updates; i++) {
			...
			if (state.getServerBpp() != bitsperpixel) {
				JulLog.warn("Server limited colour depth to " + bitsperpixel + " bits");
				state.setServerBpp(bitsperpixel);
			}
			if (compression == 0) {
				// JulLog.info("compression == 0");
			...
			if ((compression & 0x400) != 0) {
				// JulLog.info("compression & 0x400 != 0");
				size = buffersize;
			} else {
				// JulLog.info("compression & 0x400 == 0");
	...
	protected void processSaveSessionInfo(Packet data) {
		...
		context.setLoggedOn();
		JulLog.info("TODO SVE SES INF " + infoType + " : " + data);
	}
	...
	protected int processSetErrorPdu(Packet data) {
		...
		int vv = v & 0xff;
		if (JulLog.isDebugEnabled())
			JulLog.debug(String.format("Received set error PDU (%d - %d) [%s]", v, vv, data));
		return v;
	}
	...
	protected int processServerStatusPdu(Packet data) {
		int v = data.getLittleEndian32();
		if (JulLog.isDebugEnabled())
			JulLog.debug(String.format("Received server status PDU (%d) [%s]", v, data));
		return v;
	}
	...
	void processServerCaps(Packet data, int length) {
		...
		for (n = 0; n < ncapsets; n++) {
			...
			case RDP_CAPSET_VIRTUAL_CHANNELS:
				JulLog.info("Unhandled CAPSET virtual channels");
				break;
			case RDP_CAPSET_DRAWGDIPLUS:
				JulLog.info("Unhandled CAPSET draw GDP plus");
				break;
			case RDP_CAPSET_FONTS:
				JulLog.info("Unhandled CAPSET fonts");
				break;
			case RDP_CAPSET_BITMAP_CODECS:
				JulLog.info("Unhandled CAPSET bitmap codecs");
				break;
			case RDP_CAPSET_ORDER:
				JulLog.info("Unhandled CAPSET order");
				break;
			case RDP_CAPSET_COLOR_CACHE:
				JulLog.info("Unhandled CAPSET color cache");
				break;
			case RDP_CAPSET_BITMAP_CACHE_HOST:
				JulLog.info("Unhandled CAPSET bitmap cache host");
				break;
			case RDP_CAPSET_LARGE_POINTER:
				if (capset_length >= RDP_CAPLEN_LARGE_POINTER) {
					serverLargePointerFlags = data.getLittleEndian16();
					JulLog.info("Server large-pointer flags=0x"
							+ Integer.toHexString(serverLargePointerFlags));
				}
				break;
			case RDP_CAPSET_INPUT:
				JulLog.info("Unhandled CAPSET input");
				break;
			case RDP_CAPSET_RAIL:
				JulLog.info("Unhandled CAPSET rail");
				break;
			case RDP_CAPSET_WINDOW_LIST:
				JulLog.info("Unhandled CAPSET window list");
				break;
			case RDP_CAPSET_COMPDESK:
				JulLog.info("Unhandled CAPSET compdesk");
				break;
			case RDP_CAPSET_MULTIFRAGMENT_UPDATE:
				if (capset_length >= RDP_CAPLEN_MULTIFRAGMENT_UPDATE) {
					...
					JulLog.info("Server multifragment max request size="
							+ Integer.toUnsignedString(serverMultifragmentMaxRequestSize));
				}
				break;
			case RDP_CAPSET_SURFACE_COMMANDS:
				JulLog.info("Unhandled CAPSET surface commands");
				break;
			default:
				JulLog.warn("Unhandled CAPSET " + capset_type);
				break;
			}
	...
	private Packet initData(int size) throws RdesktopException {
		Packet buffer = null;
		buffer = secureLayer.init(state.getSecurityType() == SecurityType.STANDARD ? com.tangluobo.rdp4j.layers.Secure.SEC_ENCRYPT : 0, size + 18);
	...
	private void process_system_pointer_pdu(Packet data) {
		...
		case RDP_DEFAULT_POINTER:
			...
			break;
		default:
			JulLog.warn("Unimplemented system pointer message 0x" + Integer.toHexString(system_pointer_type));
	...
	private void processData(Packet data) throws RdesktopException, OrderException {
		...
		case (Rdp.RDP_DATA_PDU_UPDATE):
			if (JulLog.isDebugEnabled())
				JulLog.debug("Rdp.RDP_DATA_PDU_UPDATE");
			this.processUpdate(data);
			break;
		case RDP_DATA_PDU_CONTROL:
			if (JulLog.isDebugEnabled())
				JulLog.debug(("Received Control PDU\n"));
			break;
		case RDP_DATA_PDU_SYNCHRONISE:
			if (JulLog.isDebugEnabled())
				JulLog.debug(("Received Sync PDU\n"));
			break;
		case (Rdp.RDP_DATA_PDU_POINTER):
			if (JulLog.isDebugEnabled())
				JulLog.debug("Received pointer PDU");
		...
		case PDUTYPE2_STATUS_INFO_PDU:
			...
			break;
		default:
			JulLog.warn("Unimplemented Data PDU type " + data_type);
			break;
		}
	}
	...
	private void processDemandActive(Packet data) throws RdesktopException, IOException {
		...
		JulLog.info(String.format("Share ID is %d (%04x)", state.getShareId(), state.getShareId()));
		...
		if (JulLog.isDebugEnabled())
			JulLog.debug("RDP_PDU_SYNCHRONIZE " + p);
		p = this.receive(type); // Receive RDP_CTL_COOPERATE
		if (JulLog.isDebugEnabled())
			JulLog.debug("RDP_CTL_COOPERATE " + p);
		p = this.receive(type); // Receive RDP_CTL_GRANT_CONTROL
		if (JulLog.isDebugEnabled())
			JulLog.debug("RDP_CTL_GRANT_CONTROL " + p);
		p = this.receive(type); // Receive TS_FONT_MAP_PDU (0x28)
		if (JulLog.isDebugEnabled())
			JulLog.debug("TS_FONT_MAP_PDU " + p);
		JulLog.info("reset order state");
	...
	private void processPointer(Packet data) throws RdesktopException {
		...
		case (Rdp.RDP_POINTER_MOVE):
			if (JulLog.isDebugEnabled())
				JulLog.debug("Rdp.RDP_POINTER_MOVE");
			x = data.getLittleEndian16();
			y = data.getLittleEndian16();
	...
	private void processUpdate(Packet data) throws OrderException, RdesktopException {
		...
		case (Rdp.RDP_UPDATE_SYNCHRONIZE):
			break;
		default:
			JulLog.warn("Unimplemented Update type " + update_type);
		}
	}
	...
	private Packet receive(int[] type) throws IOException, RdesktopException {
		...
		if (length == 0x8000) {
			JulLog.warn("32k packet keepalive fix");
			next_packet += 8;
			type[0] = 0;
			return stream;
		}
	...
	private void sendBitmapcache2Caps(Packet data) {
		...
		if (state.getOptions().getPersistentCacheBackend().init(2)) {
			JulLog.info("Persistent cache initialized");
			data.setLittleEndian32(BMPCACHE2_NUM_PSTCELLS | BMPCACHE2_FLAG_PERSIST);
		} else {
			JulLog.info("Persistent cache not initialized");
	...
	private void sendConfirmActive() throws RdesktopException, IOException {
		...
		if (enableLargePointer) {
			JulLog.info("Advertising large-pointer flags=0x{} with multifragment max request size={}",
		...
		// Purpose
		// unknown
		int sec_flags = state.getSecurityType() == SecurityType.STANDARD ? RDP5_FLAG | com.tangluobo.rdp4j.layers.Secure.SEC_ENCRYPT : RDP5_FLAG;
		...
		if (state.isRDP5() && state.getOptions().getPersistentCacheBackend() != null) {
			JulLog.info("Persistent caching enabled");
	...
	private void sendData(Packet data, int data_pdu_type) throws RdesktopException, IOException {
		try {
			...
			try {
				...
				secureLayer.send(data, state.getSecurityType() == SecurityType.STANDARD ? com.tangluobo.rdp4j.layers.Secure.SEC_ENCRYPT : 0);
			} finally {
	...
	private void sendGeneralCaps(Packet data) {
		JulLog.info("Sending general caps");
		...
		JulLog.info(String.format("     OS Major: %d", state.getOptions().getOsMajor()));
		data.setLittleEndian16(
				state.getOptions().getOsMinor()); /* OS minor type */
		JulLog.info(String.format("     OS Minor: %d", state.getOptions().getOsMinor()));
	...
	private void sendLogonInfo(String command, String directory) throws RdesktopException, IOException {
		...
		if (rdstlsCredentials != null) {
			...
			if (creds == null) {
				...
				domain = StringUtil.blankToDefault(creds.get(0), "");
				username = StringUtil.blankToDefault(creds.get(1), "");
				password = StringUtil.blankToDefault(creds.get(2), "").toCharArray();
			}
		}
		directory = StringUtil.blankToDefault(directory, "");
		...
		JulLog.info("Sending Client Info: account=" + (account.isEmpty() ? "(empty)" : account)
		...
		int sec_flags = state.getSecurityType() == SecurityType.STANDARD ? com.tangluobo.rdp4j.layers.Secure.SEC_LOGON_INFO | com.tangluobo.rdp4j.layers.Secure.SEC_ENCRYPT
		...
		if (JulLog.isDebugEnabled())
			JulLog.debug("Sending RDP Logon packet");
		...
		if (state.isRDP5()) {
			if (JulLog.isDebugEnabled())
				JulLog.debug("Sending RDP5+ extra login info");
			/* Extra Info */
	...
	static boolean processBitmapCaps(State options, Packet data) {
		...
		if (JulLog.isDebugEnabled())
			JulLog.debug("setting desktop size and bpp to: " + width + "x" + height + "x" + bpp);
		/*
		 * The server may limit bpp and change the size of the desktop (for
		 * example when shadowing another session).
		 */
		if (options.getServerBpp() != bpp) {
			JulLog.warn("colour depth changed from " + options.getServerBpp() + " to " + bpp);
	...
	static void processShareCaps(State state, Packet data) {
		...
		JulLog.info(String.format("Server channel is %d (%04x)", state.getServerChannelId(), state.getServerChannelId()));
	}
