...
public class Transport implements Layer<com.tangluobo.rdp4j.layers.ISO> {
	...
	private com.tangluobo.rdp4j.layers.ISO iso;

	public Transport(State state, com.tangluobo.rdp4j.layers.ISO iso) {
		this.state = state;
		this.iso = iso;
	}
	...
	public void startSsl() throws IOException, RdesktopException {
		try {
			...
			JulLog.info("SSL handshake completed.");
		} catch (Exception e) {
	...
	public Packet receivePacket(Packet p, int length) throws IOException {
		...
		try {
			...
			if (JulLog.isDebugEnabled()) {
				JulLog.debug(String.format("I/O error reading packet. Expected up to %d bytes.", length));
