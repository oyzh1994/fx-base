...
public class FileCacheBackend implements CacheBackend {
	...
	public void start(com.tangluobo.rdp4j.State state) {
		this.state = state;
	}
	...
	public void touchBitmap(int cache_id, int cache_idx, int stamp) {
		JulLog.info("PstCache.touchBitmap");
		FileOutputStream fd;
	...
	public int pstcache_enumerate(com.tangluobo.rdp4j.State state, int cache_id, int[] idlist) throws IOException, com.tangluobo.rdp4j.RdesktopException {
		JulLog.info("PstCache.pstcache_enumerate");
		FileInputStream fd;
		int n, c = 0;
		...
		if (JulLog.isDebugEnabled())
			JulLog.debug("pstcache enumeration... ");
		...
		JulLog.info(n + " bitmaps in persistent cache, " + c + " bitmaps loaded in memory\n");
	...
	public boolean init(int cache_id) {
		...
		if (JulLog.isDebugEnabled())
			JulLog.debug("persistent bitmap cache file: " + filename);
		File cacheDir = new File("./cache/");
		if (!cacheDir.exists() && !cacheDir.mkdir()) {
			JulLog.warn("failed to get/make cache directory");
			return false;
		}
		File f = new File(filename);
		try {
			if (!f.exists() && !f.createNewFile()) {
				JulLog.warn("Could not create cache file");
				return false;
			}
		} catch (IOException e) {
			return false;
		...
		/*
		 * if (!rd_lock_file(fd, 0, 0)) {JulLog.warn(
	...
	public boolean pstcache_load_bitmap(com.tangluobo.rdp4j.State state, int cache_id, int cache_idx) throws IOException, RdesktopException {
		JulLog.info("PstCache.pstcache_load_bitmap");
		byte[] celldata = null;
		FileInputStream fd;
		// CELLHEADER cellhdr;
		Bitmap bitmap;
		...
		fd.read(celldata);
		if (JulLog.isDebugEnabled())
			JulLog.debug("Loading bitmap from disk (" + cache_id + ":" + cache_idx + ")\n");
	...
	public boolean pstcache_put_bitmap(State state, int cache_id, int cache_idx, byte[] bitmap_id, int width, int height,
		...
		JulLog.info("PstCache.pstcache_put_bitmap");
		FileOutputStream fd;
