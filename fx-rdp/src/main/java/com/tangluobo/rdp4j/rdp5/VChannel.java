...
public abstract class VChannel {
	...
	public void send_packet(Packet data) throws RdesktopException, IOException {
		...
		int data_offset = 0;
		int num_packets = (length / com.tangluobo.rdp4j.rdp5.VChannels.CHANNEL_CHUNK_LENGTH);
		num_packets += length - (com.tangluobo.rdp4j.rdp5.VChannels.CHANNEL_CHUNK_LENGTH) * num_packets;
		while (data_offset < length) {
			int thisLength = Math.min(com.tangluobo.rdp4j.rdp5.VChannels.CHANNEL_CHUNK_LENGTH, length - data_offset);
			...
			int flags = ((data_offset == 0) ? com.tangluobo.rdp4j.rdp5.VChannels.CHANNEL_FLAG_FIRST : 0);
			if (data_offset + thisLength >= length)
				flags |= com.tangluobo.rdp4j.rdp5.VChannels.CHANNEL_FLAG_LAST;
			if ((this.flags() & com.tangluobo.rdp4j.rdp5.VChannels.CHANNEL_OPTION_SHOW_PROTOCOL) != 0)
