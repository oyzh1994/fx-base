...
public class RdpIsoFix {
    ...
    public static class RdpIso extends ISO {
        ...
        public Packet receive() throws IOException, RdesktopException {
            ...
            if (!firstCallLogged) {
                ...
                receiveCalled = true;
                JulLog.info("[RdpIso] receive()首次调用 - RdpIso已激活, transport=" + transport.getClass().getName());
            }
            ...
            JulLog.info("[SLOW-PATH] Non-DT X.224 type=0x" + String.format("%02x", type));
    ...
    public static void injectRdpIso(Object rdpLayer) {
        try {
            ...
            JulLog.info("已替换ISO为RdpIso（修复fast-path检测条件 (version&1)==0）");
            injected = true;
