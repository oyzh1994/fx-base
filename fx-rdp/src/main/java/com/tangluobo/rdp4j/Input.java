...
public class Input implements RdpInput {
	...
	public boolean handleShortcutKeys(long time, KeyEvent e, boolean pressed) {
		...
		case KeyEvent.VK_PAGE_DOWN: // Alt + PgDown = Alt-Shift-Tab
			if (pressed) {
				...
				if (JulLog.isDebugEnabled())
					JulLog.debug("shortcut pressed: sent ALT+SHIFT+TAB");
			} else {
		...
		case KeyEvent.VK_INSERT: // Alt + Insert = Alt + Esc
			...
			if (pressed)
				if (JulLog.isDebugEnabled())
					JulLog.debug("shortcut pressed: sent ALT+ESC");
			break;
		case KeyEvent.VK_HOME: // Alt + Home = Ctrl + Esc (Start)
			if (pressed) {
				...
				if (JulLog.isDebugEnabled())
					JulLog.debug("shortcut pressed: sent CTRL+ESC (Start)");
			} else {
		...
		case KeyEvent.VK_END: // Ctrl+Alt+End = Ctrl+Alt+Del
			if (ctrlDown) {
				...
				if (pressed)
					if (JulLog.isDebugEnabled())
						JulLog.debug("shortcut pressed: sent CTRL+ALT+DEL");
			}
			break;
		case KeyEvent.VK_DELETE: // Alt + Delete = Menu
			if (pressed) {
				...
				if (JulLog.isDebugEnabled())
					JulLog.debug("shortcut pressed: sent MENU");
			} else {
		...
		case KeyEvent.VK_SUBTRACT: // Ctrl + Alt + Minus (on NUM KEYPAD) =
			...
			if (ctrlDown) {
				if (pressed) {
					...
					if (JulLog.isDebugEnabled())
						JulLog.debug("shortcut pressed: sent ALT+PRTSC");
				} else {
		...
		case KeyEvent.VK_EQUALS: // for laptops that can't do Ctrl-Alt+Plus
			if (ctrlDown) {
				if (pressed) {
					...
					sendScancode(time, RDP_KEYPRESS, 0x37 | KeyCode.SCANCODE_EXTENDED); // PrtSc
					if (JulLog.isDebugEnabled())
						JulLog.debug("shortcut pressed: sent PRTSC");
				} else {
					sendScancode(time, RDP_KEYRELEASE, 0x37 | KeyCode.SCANCODE_EXTENDED); // PrtSc
					sendScancode(time, RDP_KEYPRESS, 0x1d); // Ctrl
					sendScancode(time, RDP_KEYPRESS, 0x38); // Alt
	...
	public boolean handleSpecialKeys(long time, KeyEvent e, boolean pressed) {
		...
		/*
		 * case KeyEvent.VK_META: // Windows key JulLog.debug("Windows key
	...
	public void sendScancode(long time, int flags, int scancode) {
		if (scancode == 0x38) { // be careful with alt
			if ((flags & RDP_KEYRELEASE) != 0) {
				// JulLog.info("Alt release, serverAltDown = " + serverAltDown);
				serverAltDown = false;
			}
			if ((flags == RDP_KEYPRESS)) {
				// JulLog.info("Alt press, serverAltDown = " + serverAltDown);
				serverAltDown = true;
			}
		}
	...
	public void triggerReadyToSend() {
		JulLog.info("Input ready to send");
		remoteShiftStateKnown = false;
		synchronizeLockKeys();
	...
	public void synchronizeLockKeys() {
		try {
			...
			JulLog.warn("Local lock-key state is unavailable; skipping keyboard LED synchronization");
		}
	}
	...
	protected void doLockKeys() {
		...
		JulLog.info("Synchronizing keyboard toggle flags: 0x" + Integer.toHexString(toggleFlags)
	...
	protected void middleButtonPressed(MouseEvent e) {
		 ...
		 * }catch (RdesktopException ex){ JulLog.warn(ex.getMessage()); } if
		 * (capsLockOn){ JulLog.debug("Turning caps lock off for paste"); //
		 ...
		 * JulLog.debug("Turning caps lock back on after paste");
	...
	class RdesktopKeyAdapter extends KeyAdapter {
		...
		public void keyPressed(KeyEvent e) {
			...
			if (JulLog.isDebugEnabled())
				JulLog.debug("PRESSED keychar='" + e.getKeyChar() + "' keycode=0x" + Integer.toHexString(e.getKeyCode()) + " char='"
						+ ((char) e.getKeyCode()) + "'");
			if (state.getRdp() != null) {
		...
		public void keyReleased(KeyEvent e) {
			...
			long time = getTime();
			if (JulLog.isDebugEnabled())
				JulLog.debug("RELEASED keychar='" + e.getKeyChar() + "' keycode=0x" + Integer.toHexString(e.getKeyCode())
		...
		public void keyTyped(KeyEvent e) {
			...
			pressedKeys.addElement(new Integer(e.getKeyCode()));
			if (JulLog.isDebugEnabled())
				JulLog.debug("TYPED keychar='" + e.getKeyChar() + "' keycode=0x" + Integer.toHexString(e.getKeyCode()) + " char='"
						+ ((char) e.getKeyCode()) + "'");
			if (state.getRdp() != null) {
				if (!handleSpecialKeys(time, e, true))
					sendKeyPresses(state.getOptions().getKeymap().getKeyStrokes(e));
		...
		public void mousePressed(MouseEvent e) {
			...
			if (state.getRdp() != null) {
				...
				if ((e.getModifiers() & InputEvent.BUTTON1_MASK) == InputEvent.BUTTON1_MASK) {
					...
					if (JulLog.isDebugEnabled())
						JulLog.debug("Middle Mouse Button Pressed.");
	...
	class RdesktopMouseMotionAdapter extends MouseMotionAdapter {
		...
		public void mouseDragged(MouseEvent e) {
			int time = getTime();
			// if(logger.isInfoEnabled()) JulLog.info("mouseMoved to
	...
	private class RdesktopMouseWheelAdapter implements MouseWheelListener {
		...
		public void mouseWheelMoved(MouseWheelEvent e) {
			int time = getTime();
			// if(logger.isInfoEnabled()) JulLog.info("mousePressed at "+time);
			if (state.getRdp() != null) {
				reconcileRemoteShift(e.isShiftDown());
