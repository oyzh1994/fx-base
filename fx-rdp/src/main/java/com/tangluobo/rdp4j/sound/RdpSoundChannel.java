 ...
 * 3. SNDC_WAVE/SNDC_WAVE2音频数据块 → 交{@link com.tangluobo.rdp4j.sound.RdpAudioPlayer}播放；
...
public class RdpSoundChannel extends VChannel {
    ...
    // ===== 状态 =====
    private final com.tangluobo.rdp4j.sound.RdpAudioPlayer player = new RdpAudioPlayer();
    /**
     * 服务器通告的格式
     */
        ...
        try {
            ...
            switch (msgType) {
                ...
                case SNDC_SETVOLUME:
                    ...
                    break;
                case SNDC_CLOSE:
                    JulLog.info("服务器关闭音频重定向");
                    break;
                default:
                    JulLog.trace("rdpsnd忽略消息: msgType=0x" + Integer.toHexString(msgType)
                            + " bodySize=" + bodySize);
                    break;
            }
        } catch (Exception e) {
            // 音频异常不应断开会话：记录并继续
            JulLog.warn("rdpsnd消息处理异常: " + e.getMessage(), e);
        }
    }
    ...
    private void recvServerFormats(Packet packet) throws RdesktopException, IOException {
        ...
        for (int i = 0; i < numFormats; i++) {
            ...
            if (entry == null) {
                JulLog.warn("音频格式解析失败（第" + i + "项），停止解析剩余格式");
                break;
            }
        ...
        JulLog.info("服务器音频格式: " + serverFormats.size() + "个，可播放: " + clientFormats.size()
    ...
    private void sendClientFormats() throws RdesktopException, IOException {
        ...
        if (clientFormats.isEmpty()) {
            JulLog.warn("无可播放的音频格式，远程音频将被禁用");
        } else {
            AudioFormatEntry first = clientFormats.get(0);
            JulLog.info(String.format("已应答音频格式: %d个，首个: PCM %dHz %dbit %d声道",
    ...
    private void recvTraining(Packet packet) throws RdesktopException, IOException {
        ...
        p.markEnd();
        sendWithLock(p);
        JulLog.trace("已回显训练消息: wTimeStamp=" + timeStamp + " wPackSize=" + packSize);
    }
    ...
    private void recvWaveInfo(Packet packet, int bodySize) {
        ...
        if (waveDataSize < 4 || waveFormatNo >= clientFormats.size()) {
            JulLog.warn("无效Wave Info: formatNo=" + waveFormatNo + " waveDataSize=" + waveDataSize);
            return;
        }
        expectingWave = true;
    }
    ...
    private void recvWave2(Packet packet, int bodySize) {
        ...
        if (waveFormatNo >= clientFormats.size()) {
            JulLog.warn("无效Wave2: formatNo=" + waveFormatNo);
            return;
        }
    ...
    private void sendWaveConfirm(int timeStamp) {
        try {
            ...
            JulLog.error("发送WaveConfirm失败: " + e.getMessage());
        }
    }
