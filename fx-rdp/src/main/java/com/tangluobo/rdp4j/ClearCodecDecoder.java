...
final class ClearCodecDecoder {
    ...
    void decode(byte[] data, int offset, int length, int width, int height,
        ...
        if (sequence != expectedSequence) {
            throw new com.tangluobo.rdp4j.RdesktopException("ClearCodec序号不连续: 收到=" + sequence
                    + ", 期望=" + expectedSequence);
        }
        ...
        if ((flags & FLAG_GLYPH_HIT) != 0 && (flags & FLAG_GLYPH_INDEX) == 0) {
            throw new com.tangluobo.rdp4j.RdesktopException("ClearCodec字形命中缺少缓存索引");
        }
        ...
        if ((flags & FLAG_GLYPH_INDEX) != 0) {
            ...
            if (glyphIndex >= GLYPH_CACHE_SIZE) {
                throw new com.tangluobo.rdp4j.RdesktopException("ClearCodec字形缓存索引越界: " + glyphIndex);
            }
        }
        ...
        if ((flags & FLAG_GLYPH_HIT) != 0) {
            ...
            if (glyph == null || glyph.length < pixelCount) {
                throw new com.tangluobo.rdp4j.RdesktopException("ClearCodec引用了不存在的字形缓存: " + glyphIndex);
            }
        ...
        if (input.remaining() < 12) {
            throw new com.tangluobo.rdp4j.RdesktopException("ClearCodec组合数据头不完整");
        }
        ...
        if (compositionLength != input.remaining()) {
            throw new com.tangluobo.rdp4j.RdesktopException("ClearCodec组合数据长度不匹配: 声明="
        ...
        if (glyphIndex >= 0 && (flags & FLAG_GLYPH_HIT) == 0) {
            ...
            if (pixelCount > MAX_GLYPH_PIXELS) {
                throw new com.tangluobo.rdp4j.RdesktopException("ClearCodec字形过大: " + width + "x" + height);
            }
    ...
    private static void decodeResidual(Cursor input, int width, int height,
        ...
        while (input.remaining() > 0) {
            ...
            if (runLength > pixelCount - pixelIndex) {
                throw new com.tangluobo.rdp4j.RdesktopException("ClearCodec残差游程越过目标位图");
            }
        ...
        if (pixelIndex != pixelCount) {
            throw new com.tangluobo.rdp4j.RdesktopException("ClearCodec残差像素数不匹配: " + pixelIndex
                    + "/" + pixelCount);
        }
    }
    ...
    private void decodeBands(Cursor input, int width, int height,
        ...
        while (input.remaining() > 0) {
            ...
            if (xEnd < xStart || yEnd < yStart || xEnd >= width || yEnd >= height) {
                throw new com.tangluobo.rdp4j.RdesktopException("ClearCodec条带区域越界: " + xStart + "," + yStart
                        + "-" + xEnd + "," + yEnd + "，位图=" + width + "x" + height);
            }
            int barHeight = yEnd - yStart + 1;
            if (barHeight > MAX_VBAR_HEIGHT) {
                throw new com.tangluobo.rdp4j.RdesktopException("ClearCodec垂直条高度过大: " + barHeight);
            }
            ...
            for (int column = 0; column < barCount; column++) {
                ...
                if ((header & 0xC000) == 0x4000) {
                    ...
                    if (shortBar == null) {
                        throw new com.tangluobo.rdp4j.RdesktopException("ClearCodec引用了不存在的短条缓存: " + index);
                    }
                    int yOn = input.u8();
                    ...
                    if (yOff < yOn) {
                        throw new com.tangluobo.rdp4j.RdesktopException("ClearCodec短条范围无效: " + yOn + "-" + yOff);
                    }
                    int count = yOff - yOn;
                    if (count > MAX_VBAR_HEIGHT) {
                        throw new com.tangluobo.rdp4j.RdesktopException("ClearCodec短条像素数过大: " + count);
                    }
                    ...
                    throw new com.tangluobo.rdp4j.RdesktopException(String.format(
    ...
    private static void decodeSubcodecs(Cursor input, int width, int height,
        ...
        while (input.remaining() > 0) {
            ...
            if (subWidth == 0 || subHeight == 0
                ...
                throw new com.tangluobo.rdp4j.RdesktopException("ClearCodec子编码区域越界: " + x + "," + y
            ...
            switch (codecId) {
                ...
                case 2:
                    ...
                    break;
                default:
                    throw new com.tangluobo.rdp4j.RdesktopException("ClearCodec子编码类型不支持: " + codecId);
            }
    ...
    private static void decodeRaw(Cursor input, int width, int height,
        ...
        if (expected != input.remaining()) {
            throw new com.tangluobo.rdp4j.RdesktopException("ClearCodec原始子位图长度不匹配: " + input.remaining()
                    + "/" + expected);
        }
    ...
    private static void decodeRlex(Cursor input, int width, int height,
        ...
        if (paletteCount < 1 || paletteCount > 127) {
            throw new com.tangluobo.rdp4j.RdesktopException("ClearCodec RLEX调色板大小无效: " + paletteCount);
        }
        ...
        while (input.remaining() > 0) {
            ...
            if (start < 0 || stop >= paletteCount) {
                throw new com.tangluobo.rdp4j.RdesktopException("ClearCodec RLEX调色板索引越界: "
                        + start + "-" + stop + "/" + paletteCount);
            }
            if (runLength > pixelCount - pixelIndex) {
                throw new com.tangluobo.rdp4j.RdesktopException("ClearCodec RLEX游程越过目标位图");
            }
            ...
            if (suiteLength > pixelCount - pixelIndex) {
                throw new com.tangluobo.rdp4j.RdesktopException("ClearCodec RLEX色组越过目标位图");
            }
        ...
        if (pixelIndex != pixelCount) {
            throw new com.tangluobo.rdp4j.RdesktopException("ClearCodec RLEX像素数不匹配: " + pixelIndex
                    + "/" + pixelCount);
        }
    }
    ...
    private static void decodeNsc(Cursor input, int width, int height,
        ...
        if (input.remaining() < 20) {
            throw new com.tangluobo.rdp4j.RdesktopException("NSCodec数据头不完整");
        }
        ...
        if (colorLossLevel < 1 || colorLossLevel > 7) {
            throw new com.tangluobo.rdp4j.RdesktopException("NSCodec颜色损失级别无效: " + colorLossLevel);
        }
        if (chromaSubsampling != 0 && chromaSubsampling != 1) {
            throw new com.tangluobo.rdp4j.RdesktopException("NSCodec色度抽样级别无效: " + chromaSubsampling);
        }
        if (encodedLength != input.remaining()) {
            throw new com.tangluobo.rdp4j.RdesktopException("NSCodec平面长度不匹配: 声明=" + encodedLength
                    + ", 实际=" + input.remaining());
        }
        ...
        if (chromaSubsampling != 0) {
            ...
            if (yLength > Integer.MAX_VALUE || chromaLength > Integer.MAX_VALUE) {
                throw new com.tangluobo.rdp4j.RdesktopException("NSCodec抽样平面尺寸过大");
            }
    ...
    private static byte[] decodeNscPlane(Cursor input, int encodedLength, int originalLength)
        ...
        while (left > 4) {
            ...
            if (left == 5) {
                ...
                if (runLength > left || runLength > output.length - outputPosition) {
                    throw new com.tangluobo.rdp4j.RdesktopException("NSCodec RLE游程越过目标平面");
                }
        ...
        if (outputPosition != originalLength) {
            throw new com.tangluobo.rdp4j.RdesktopException("NSCodec RLE解码长度不匹配: " + outputPosition
                    + "/" + originalLength);
        }
        return output;
    }
    ...
    private static void validateDestination(int width, int height, int[] destination,
            int destinationWidth, int destinationX, int destinationY) throws com.tangluobo.rdp4j.RdesktopException {
        ...
        if (pixelCount == 0 || destination == null || destinationWidth <= 0
            ...
            throw new com.tangluobo.rdp4j.RdesktopException("ClearCodec目标位图无效");
        }
        ...
        if (destinationX < 0 || destinationY < 0
            ...
            throw new com.tangluobo.rdp4j.RdesktopException("ClearCodec目标区域越界: " + destinationX + ","
                    + destinationY + "+" + width + "x" + height);
        }
    }

    private static int checkedPixels(int width, int height, String name) throws com.tangluobo.rdp4j.RdesktopException {
        long count = (long) width * height;
        if (width <= 0 || height <= 0 || count > Integer.MAX_VALUE) {
            throw new com.tangluobo.rdp4j.RdesktopException(name + "尺寸无效: " + width + "x" + height);
        }
        return (int) count;
    }
    ...
    private static final class Cursor {
        ...
        Cursor(byte[] data, int offset, int length, String name) throws com.tangluobo.rdp4j.RdesktopException {
            if (data == null || offset < 0 || length < 0 || offset > data.length - length) {
                throw new com.tangluobo.rdp4j.RdesktopException(name + "数据不完整");
            }
            this.data = data;
        ...
        int u8() throws com.tangluobo.rdp4j.RdesktopException {
            require(1);
            return data[position++] & 0xFF;
        }

        int peekU8() throws com.tangluobo.rdp4j.RdesktopException {
            require(1);
            return data[position] & 0xFF;
        }

        int u16() throws com.tangluobo.rdp4j.RdesktopException {
            require(2);
        ...
        long u32() throws com.tangluobo.rdp4j.RdesktopException {
            require(4);
        ...
        int bgr() throws com.tangluobo.rdp4j.RdesktopException {
            int blue = u8();
            int green = u8();
            int red = u8();
        ...
        long runLength() throws com.tangluobo.rdp4j.RdesktopException {
            long run = u8();
            if (run == 0xFF) {
                run = u16();
                if (run == 0xFFFF) {
        ...
        int length32(String field) throws com.tangluobo.rdp4j.RdesktopException {
            long value = u32();
            if (value > Integer.MAX_VALUE) {
                throw new com.tangluobo.rdp4j.RdesktopException(field + "长度过大: " + value);
            }
            return (int) value;
        }

        Cursor section(int length, String sectionName) throws com.tangluobo.rdp4j.RdesktopException {
            require(length);
        ...
        void requireFinished() throws com.tangluobo.rdp4j.RdesktopException {
            if (position != end) {
                throw new com.tangluobo.rdp4j.RdesktopException(name + "含有未解析数据: " + remaining() + "字节");
            }
        }

        private void require(int length) throws com.tangluobo.rdp4j.RdesktopException {
