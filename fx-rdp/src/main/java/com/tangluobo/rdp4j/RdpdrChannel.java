...
final class RdpdrChannel extends VChannel {
    ...
    public void process(Packet data) throws RdesktopException, IOException {
        ...
        if (available < 4) {
            JulLog.warn("rdpdr: truncated packet, bytes=" + available);
            return;
        }
        ...
        switch (packetId) {
            ...
            case PAKID_CORE_CLIENTID_CONFIRM:
                ...
                JulLog.info("rdpdr: 服务端已确认ClientId");
                maybeSendEmptyDeviceList();
                break;
    ...
    private void processServerAnnounce(Packet data, int bodyLength)
        ...
        if (bodyLength < 8) {
            JulLog.warn("rdpdr: Server Announce packet too short");
            return;
        }
        ...
        sendClientName();
        JulLog.info("rdpdr: 初始化应答已发送，serverVersion=" + serverMajor
    ...
    private void sendClientCapabilities() throws RdesktopException, IOException {
        ...
        p.markEnd();
        send_packet(p);
        JulLog.info("rdpdr: Client Core Capability Response已发送（无设备能力）");
    }
    ...
    private void sendEmptyDeviceList() throws RdesktopException, IOException {
        ...
        JulLog.info("rdpdr: 空设备列表已发送，核心初始化完成");
    }
