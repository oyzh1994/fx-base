...
public class FixedClipChannel extends ClipChannel {
    ...
    private final com.tangluobo.rdp4j.clipboard.FixedUnicodeHandler unicodeHandler = new FixedUnicodeHandler();
     ...
     */
    private volatile com.tangluobo.rdp4j.clipboard.WindowsVirtualFileClipboard windowsVirtualClipboard;
    /**
    ...
    private void replaceUnicodeHandler() {
        try {
            ...
            if (allHandlers == null) {
                JulLog.warn("allHandlers未初始化，跳过UnicodeHandler替换");
                return;
            }
            ...
            if (replacedFlag) {
                ...
                JulLog.info("UnicodeHandler已替换为UTF-16LE修复版");
            }
        } catch (Exception e) {
            JulLog.error("替换UnicodeHandler失败: " + e.getMessage(), e);
        }
    }
        ...
        try {
            ...
            for (int i = 0; i < count && remaining >= 4; i++) {
                ...
                if (capType == 1 && capLen >= 12) {
                    ...
                    JulLog.info("服务器cliprdr能力: generalFlags=0x" + Integer.toHexString(serverGeneralFlags));
                    remaining -= 8;
                } else {
            ...
            JulLog.error("解析服务器cliprdr能力失败: " + e.getMessage(), e);
        }
    }
    ...
    public void synchronizeLocalClipboard() {
        try {
            ...
            JulLog.error("剪贴板焦点同步失败: " + ex.getMessage(), ex);
        }
    }
    ...
    private void sendCapsIfNeeded() {
        ...
        try {
            ...
            p.markEnd();
            send_packet(p);
            JulLog.trace("已发送cliprdr能力协商（长格式名+文件流）");
        } catch (Exception e) {
            JulLog.error("发送cliprdr能力协商失败: " + e.getMessage(), e);
        }
    }
    ...
    private void announceLocalFormats() {
        try {
            com.tangluobo.rdp4j.clipboard.WindowsVirtualFileClipboard nativeClipboard = windowsVirtualClipboard;
            ...
            if (local.hasFiles()) {
                try {
                    ...
                    JulLog.error("读取本地剪贴板文件列表失败: " + e.getMessage(), e);
                }
            }
            ...
            send_packet(p);
            if (hasFiles) {
                JulLog.info("已通告本地剪贴板文件（" + announcedFiles.size() + "项，长格式名）");
            }
        } catch (Exception e) {
            JulLog.error("通告本地剪贴板格式失败: " + e.getMessage(), e);
        }
    }
    ...
    private void sendFileGroupDescriptor() {
        try {
            ...
            JulLog.error("发送文件描述列表失败: " + e.getMessage(), e);
        }
    }
    ...
    private void handleFileContentsRequest(Packet packet) {
        try {
            ...
            if (files == null || listIndex < 0 || listIndex >= files.size()) {
                JulLog.warn("FILECONTENTS_REQUEST索引越界: " + listIndex);
                return;
            }
            ...
            JulLog.error("处理文件内容请求失败: " + e.getMessage(), e);
        }
    }
    ...
    private void sendFileContentsRange(int streamId, FlatLocalFile f, long offset, int cbRequested)
        ...
        try (RandomAccessFile raf = new RandomAccessFile(f.file, "r")) {
            ...
            p.markEnd();
            send_packet(p);
            JulLog.trace("发送文件数据块: " + f.relPath + " offset=" + offset + " len=" + n);
        }
    }
    ...
    private void sendWholeFileContents(int lindex) {
        try {
            ...
            byte[] data = Files.readAllBytes(f.file.toPath());
            ...
            p.markEnd();
            send_packet(p);
            JulLog.info("非流式响应文件内容: " + f.relPath + " (" + data.length + "字节)");
        } catch (Exception e) {
            JulLog.error("非流式响应文件内容失败: " + e.getMessage(), e);
        }
    }
    ...
    private void sendDropEffect() {
        try {
            ...
            JulLog.error("响应DropEffect失败: " + e.getMessage(), e);
        }
    }
    ...
    private void handleFormatAnnounce(Packet packet, int dataLen, int msgFlags) {
        try {
            ...
            if (fileFormatId >= 0) {
                ...
                JulLog.info("远程剪贴板包含文件，开始拉取文件描述列表");
            } else {
                // 文本路径（原逻辑）：优先Unicode
            ...
            JulLog.error("处理远程剪贴板格式通告失败: " + e.getMessage(), e);
        }
    }
    ...
    private void parseFileGroupDescriptor(Packet packet, int dataLen) {
        ...
        try {
            ...
            JulLog.info("远程文件描述列表: " + remoteEntries.size() + "项");
            if (publishWindowsVirtualFiles()) {
                return;
            }
            startRemoteDownload();
        } catch (Exception e) {
            JulLog.error("解析远程文件描述列表失败: " + e.getMessage(), e);
            cancelRemoteDownload();
        }
    }
    ...
    private boolean publishWindowsVirtualFiles() {
        if (!com.tangluobo.rdp4j.clipboard.WindowsVirtualFileClipboard.isSupported()) {
            return false;
        }
        List<com.tangluobo.rdp4j.clipboard.WindowsVirtualFileClipboard.Entry> entries = remoteEntries.stream()
    ...
    private void handleFileContentsResponse(Packet packet, int dataLen, int msgFlags) {
        try {
            ...
            if (streamId != pendingStreamId) {
                JulLog.warn("忽略不匹配的文件内容响应: streamId=" + streamId);
                return;
            }
            int n = dataLen - 4;
            if (n > 0) {
            ...
            JulLog.error("处理远程文件数据失败: " + e.getMessage(), e);
            cancelRemoteDownload();
        }
    }
    ...
    private void finishRemoteDownload() {
        try {
            ...
            JulLog.error("写入本地剪贴板文件列表失败: " + e.getMessage(), e);
        } finally {
            closeDownloadOut();
        }
    }
