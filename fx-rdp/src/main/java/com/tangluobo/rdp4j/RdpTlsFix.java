...
public class RdpTlsFix {
    ...
    public static class RdpTransport extends Transport {
        ...
        private static volatile InputStream bcInputStream = null;
        ...
        public void sendPacket(com.tangluobo.rdp4j.Packet buffer) throws IOException {
            ...
            try {
                ...
                if (packet.length > 150) {
                    ...
                    for (int i = 7; i < packet.length - 24; i++) {
                        if (packet[i] == 0x01 && packet[i+1] == 0x00 &&
                            ...
                            modified = true;
                            JulLog.info(String.format("[CAPS-FIX] General Caps at offset %d: "
                ...
                JulLog.warn("[CAPS-FIX] 修改capabilities失败: " + e.getMessage());
            }
        ...
        protected IO negotiateSSL(IO io) throws Exception {
            JulLog.info("negotiateSSL (Bouncy Castle TLS), securityType=" + getState().getSecurityType());
            ...
            try {
                ...
                if (transportIn instanceof DataInputStream) {
                    DataInputStream din = (DataInputStream) transportIn;
                    ...
                    if (bufferedBytes > 0) {
                        ...
                        JulLog.warn("BufferedInputStream预读 " + bufferedBytes + " 字节: " + hex);
                    } else {
                        JulLog.info("BufferedInputStream无预读数据（available=0）");
                    }
                }
            } catch (Exception e) {
                JulLog.warn("检查BufferedInputStream失败: " + e.getMessage());
            }
            ...
            if (bufferedData != null && bufferedData.length > 0) {
                tlsIn = new SequenceInputStream(
                        new ByteArrayInputStream(bufferedData), io.getInputStream());
            } else {
            ...
            org.bouncycastle.tls.crypto.TlsCrypto crypto = new org.bouncycastle.tls.crypto.impl.bc.BcTlsCrypto(
                ...
                @Override
                public org.bouncycastle.tls.crypto.TlsCertificate createCertificate(short type, byte[] encoding) throws IOException {
                    JulLog.info("createCertificate(type=" + type + ", encoding.length=" + encoding.length + ")");
                    ...
                    try {
                        ...
                        if (exts != null) {
                            ...
                            if (ku != null) {
                                ...
                                JulLog.info("证书KeyUsage: bits=0x" + Integer.toHexString(bits) +
                                        " digitalSignature=" + ku.hasUsages(org.bouncycastle.asn1.x509.KeyUsage.digitalSignature));
                            } else {
                                JulLog.info("证书无KeyUsage扩展");
                            }
                        }
                    } catch (Exception e) {
                        JulLog.warn("诊断KeyUsage失败: " + e.getClass().getName() + ": " + e.getMessage());
                    }
                    ...
                    return new org.bouncycastle.tls.crypto.impl.bc.BcTlsCertificate(this, encoding) {
                        ...
                        protected boolean supportsKeyUsage(int keyUsageBits) {
                            JulLog.info("supportsKeyUsage被调用 keyUsageBits=" + keyUsageBits + ", 已绕过检查");
                            return true;
                        }
                    };
                }
            ...
            org.bouncycastle.tls.DefaultTlsClient tlsClient = new org.bouncycastle.tls.DefaultTlsClient(crypto) {
                ...
                @Override
                public org.bouncycastle.tls.TlsAuthentication getAuthentication() throws IOException {
                    return new org.bouncycastle.tls.TlsAuthentication() {
                        @Override
                        public void notifyServerCertificate(
                                org.bouncycastle.tls.TlsServerCertificate serverCertificate) throws IOException {
                            // 信任所有证书（RDP自签名证书）
                            JulLog.info("服务器证书已接收（信任所有）");
                            try {
                                ...
                                if (publicKey.length == 0) {
                                    throw new IOException("服务器证书公钥为空");
                                }
                                serverPublicKey.set(publicKey);
                                JulLog.info("已提取CredSSP通道绑定公钥: " + publicKey.length + " 字节");
                            } catch (IOException e) {
                                throw e;
                            } catch (Exception e) {
                                throw new IOException("无法从TLS证书提取CredSSP公钥", e);
                            }
                        }

                        @Override
                        public org.bouncycastle.tls.TlsCredentials getClientCredentials(
                                org.bouncycastle.tls.CertificateRequest certificateRequest) throws IOException {
                            return null; // 无客户端证书
                        }
                    };
                }
            ...
            JulLog.info("Starting Bouncy Castle TLS handshake...");
            protocol.connect(tlsClient);
            JulLog.info("Bouncy Castle TLS handshake completed");

            // 包装Bouncy Castle的流为IO对象
            final InputStream bcIn = protocol.getInputStream();
            final OutputStream bcOut = protocol.getOutputStream();
            bcInputStream = bcIn; // 保存引用用于诊断
            return new IO() {
                @Override public InputStream getInputStream() throws IOException { return bcIn; }
                @Override public OutputStream getOutputStream() throws IOException { return bcOut; }
    ...
    public static synchronized void apply(X509TrustManager trustManager) {
        ...
        try {
            ...
            JulLog.info("已设置系统属性jdk.tls.client.protocols=TLSv1,TLSv1.1,TLSv1.2");
    ...
    private static void enableRdpTlsCiphers() {
        try {
            ...
            JulLog.info("已设置TLS客户端协议为TLSv1,TLSv1.1,TLSv1.2（Security属性）");
