...
public class Orders {
	 ...
	 * @throws com.tangluobo.rdp4j.OrderException on error
	 * @throws com.tangluobo.rdp4j.RdesktopException on error
	 */
	public void processOrders(com.tangluobo.rdp4j.Packet data, int next_packet, int n_orders) throws com.tangluobo.rdp4j.OrderException, com.tangluobo.rdp4j.RdesktopException {
		int present = 0;
		// int n_orders = 0;
		...
		while (processed < n_orders) {
			...
			if ((order_flags & RDP_ORDER_STANDARD) == 0) {
				throw new com.tangluobo.rdp4j.OrderException("Order parsing failed!");
			}
	 ...
	 */
	private void drawText(Text2Order text2, int clipcx, int clipcy, int boxcx, int boxcy) throws com.tangluobo.rdp4j.RdesktopException {
		byte[] text = text2.getText();
		com.tangluobo.rdp4j.DataBlob entry = null;
		Glyph glyph = null;
		int offset = 0;
		int ptext = 0;
		...
		/*
		 * JulLog.debug("X: " + text2.getX() + " Y: " + text2.getY() +
		...
		for (int i = 0; i < length;) {
			...
			case (0xff):
				if (i + 2 < length) {
					...
					com.tangluobo.rdp4j.DataBlob db = new DataBlob(text[ptext + i + 2] & 0x000000ff, data);
	...
	private int inPresent(com.tangluobo.rdp4j.Packet data, int flags, int size) {
		int present = 0;
		int bits = 0;
		int i = 0;
	 ...
	 * @throws com.tangluobo.rdp4j.OrderException
	 */
	private void parseBounds(com.tangluobo.rdp4j.Packet data, BoundsOrder bounds) throws com.tangluobo.rdp4j.OrderException {
		int present = 0;
		present = data.get8();
		if ((present & 1) != 0) {
		...
		if (data.getPosition() > data.getEnd()) {
			throw new com.tangluobo.rdp4j.OrderException("Too far!");
		}
	}
	 ...
	 */
	private void parseBrush(com.tangluobo.rdp4j.Packet data, Brush brush, int present) {
		if ((present & 0x01) != 0)
	 ...
	 * @throws IOException
	 */
	private void process_bmpcache2(com.tangluobo.rdp4j.Packet data, int flags, boolean compressed) throws com.tangluobo.rdp4j.RdesktopException, IOException {
		Bitmap bitmap;
		int y;
		...
		if (JulLog.isDebugEnabled())
			JulLog.debug("BMPCACHE2(compr=" + compressed + ",flags=" + flags + ",cx=" + width + ",cy=" + height + ",id=" + cache_id
		...
		if (compressed) {
			...
			if (bmpdataInt == null) {
				if (JulLog.isDebugEnabled())
					JulLog.debug("Failed to decompress bitmap data");
				// xfree(bmpdata);
				return;
			}
		...
		if (bitmap != null) {
			...
			if ((flags & PERSIST) != 0)
				if (state.getOptions().getPersistentCacheBackend() == null)
					JulLog.warn("Wanted to persist bitmap but no cache backend set.");
				else
			...
			JulLog.error("process_bmpcache2: ui_create_bitmap failed");
		}
		// xfree(bmpdata);
	}
	 ...
	 * @throws com.tangluobo.rdp4j.RdesktopException
	 */
	private void processBitmapCache(com.tangluobo.rdp4j.Packet data) throws com.tangluobo.rdp4j.RdesktopException {
		...
		// JulLog.info("BMPCACHE(cx=" + width + ",cy=" + height + ",id=" +
		...
		if (Bpp == 1) {
			...
			if (pixel != null)
				...
				JulLog.warn("Failed to decompress bitmap");
		} else {
	 ...
	 * @throws com.tangluobo.rdp4j.RdesktopException
	 */
	private void processDeskSave(com.tangluobo.rdp4j.Packet data, DeskSaveOrder desksave, int present, boolean delta) throws com.tangluobo.rdp4j.RdesktopException {
	 ...
	 */
	private void processDestBlt(com.tangluobo.rdp4j.Packet data, DestBltOrder destblt, int present, boolean delta) {
		if ((present & 0x01) != 0)
		...
		// JulLog.info("opcode="+destblt.getOpcode());
	 ...
	 * @throws com.tangluobo.rdp4j.RdesktopException
	 */
	private void processFontCache(com.tangluobo.rdp4j.Packet data) throws com.tangluobo.rdp4j.RdesktopException {
		Glyph glyph = null;
	 ...
	 */
	private void processLine(com.tangluobo.rdp4j.Packet data, LineOrder line, int present, boolean delta) {
		if ((present & 0x01) != 0)
		...
		// if(logger.isInfoEnabled()) JulLog.info("Line from
		// ("+line.getStartX()+","+line.getStartY()+") to
		// ("+line.getEndX()+","+line.getEndY()+")");
		if (line.getOpcode() < 0x01 || line.getOpcode() > 0x10) {
			JulLog.warn("bad ROP2 0x" + line.getOpcode());
			return;
		}
		// now draw the line
	 ...
	 *            the source
	 */
	private void processMemBlt(com.tangluobo.rdp4j.Packet data, MemBltOrder memblt, int present, boolean delta) {
		if ((present & 0x01) != 0) {
		...
		// if(logger.isInfoEnabled()) JulLog.info("Memblt
	 ...
	 *            described as relative to the source
	 */
	private void processPatBlt(com.tangluobo.rdp4j.Packet data, PatBltOrder patblt, int present, boolean delta) {
		if ((present & 0x01) != 0)
		...
		// if(logger.isInfoEnabled()) JulLog.info("opcode="+patblt.getOpcode());
	 ...
	 *            previous set
	 */
	private void processPolyLine(com.tangluobo.rdp4j.Packet data, PolyLineOrder polyline, int present, boolean delta) {
		if ((present & 0x01) != 0)
		...
		// JulLog.info("polyline delta="+delta);
		// if(logger.isInfoEnabled()) JulLog.info("Line from
		// ("+line.getStartX()+","+line.getStartY()+") to
		// ("+line.getEndX()+","+line.getEndY()+")");
		// now draw the line
	 ...
	 * @throws com.tangluobo.rdp4j.RdesktopException
	 */
	private void processRawBitmapCache(com.tangluobo.rdp4j.Packet data) throws com.tangluobo.rdp4j.RdesktopException {
		int cache_id = data.get8();
	 ...
	 */
	private void processRectangle(com.tangluobo.rdp4j.Packet data, RectangleOrder rect, int present, boolean delta) {
		if ((present & 0x01) != 0)
	 ...
	 *            described as relative to the source
	 */
	private void processScreenBlt(com.tangluobo.rdp4j.Packet data, ScreenBltOrder screenblt, int present, boolean delta) {
		if ((present & 0x01) != 0)
		...
		// if(logger.isInfoEnabled())
		// JulLog.info("opcode="+screenblt.getOpcode());
	 ...
	 * @throws com.tangluobo.rdp4j.RdesktopException
	 */
	private void processSecondaryOrders(com.tangluobo.rdp4j.Packet data) throws OrderException, com.tangluobo.rdp4j.RdesktopException {
		int length = 0;
		int type = 0;
		int flags = 0;
		int next_order = 0;
		...
		case RDP_ORDER_RAW_BMPCACHE:
			if (JulLog.isDebugEnabled())
				JulLog.debug("Raw BitmapCache Order");
			this.processRawBitmapCache(data);
			break;
		case RDP_ORDER_COLCACHE:
			if (JulLog.isDebugEnabled())
				JulLog.debug("Colorcache Order");
			this.processColorCache(data);
			break;
		case RDP_ORDER_BMPCACHE:
			if (JulLog.isDebugEnabled())
				JulLog.debug("Bitmapcache Order");
			this.processBitmapCache(data);
			break;
		case RDP_ORDER_FONTCACHE:
			if (JulLog.isDebugEnabled())
				JulLog.debug("Fontcache Order");
		...
		case RDP_ORDER_RAW_BMPCACHE2:
			try {
				...
				throw new com.tangluobo.rdp4j.RdesktopException(e.getMessage());
			} /* uncompressed */
			break;
		case RDP_ORDER_BMPCACHE2:
			...
			break;
		default:
			JulLog.warn("Unimplemented 2ry Order type " + type);
		}
	 ...
	 * @throws com.tangluobo.rdp4j.RdesktopException
	 */
	private void processText2(com.tangluobo.rdp4j.Packet data, Text2Order text2, int present, boolean delta) throws RdesktopException {
		...
		if ((present & 0x010000) != 0) {
			...
			JulLog.warn("Unknown order state member (0x010000) in text2 order.\n");
		}
		...
		if ((present & 0x200000) != 0) {
			...
			text2.setText(text);
			/*
			 * if(logger.isInfoEnabled()) JulLog.info("X: " + text2.getX() + "
	 ...
	 *            the source
	 */
	private void processTriBlt(com.tangluobo.rdp4j.Packet data, TriBltOrder triblt, int present, boolean delta) {
		if ((present & 0x01) != 0) {
	 ...
	 */
	private static boolean parsePen(com.tangluobo.rdp4j.Packet data, Pen pen, int present) {
		if ((present & 0x01) != 0)
	 ...
	 */
	private static int setColor(com.tangluobo.rdp4j.Packet data) {
		int color = 0;
		int i = 0;
