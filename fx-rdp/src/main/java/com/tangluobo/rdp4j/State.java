...
public class State {
    ...
    public State(Options options) {
        ...
        try {
            ...
            if (StringUtil.isBlank(workstationName))
        ...
        if (StringUtil.isBlank(clientDir)) {
            if (OSUtil.isWindows())
        ...
        if (StringUtil.isBlank(clientDomain)) {
    ...
    public void setRDP5(boolean rdp5) {
        if (this.rdp5 != rdp5) {
            this.rdp5 = rdp5;
            if (rdp5)
                JulLog.info("Activating RDP5+");
        } else {
            JulLog.info("Activating RDP4");
        }
    }
    ...
    public void setServerRdpVersion(int serverRdpVersion) {
        ...
        if (serverRdpVersion == 1) {
            JulLog.info("Switching to RDP4 because of server version.");
            rdp5 = false;
        }
    }
