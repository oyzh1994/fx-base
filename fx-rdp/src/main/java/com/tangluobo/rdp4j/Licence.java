...
public class Licence {
	...
	public void process(Packet data) throws RdesktopException, IOException {
		...
		case (LICENCE_TAG_REISSUE):
			if (JulLog.isDebugEnabled())
				JulLog.debug("Presented licence was accepted!");
			break;
		case (LICENCE_TAG_ERROR):
			try {
				processError(data);
			} catch (com.tangluobo.rdp4j.RdesktopLicenseException rle) {
				switch (rle.getState()) {
				case com.tangluobo.rdp4j.RdesktopLicenseException.LICENSE_STATE_NO_TRANSITION:
					JulLog.info("License OK");
			...
			break;
		default:
			JulLog.warn("got licence tag: " + tag);
		}
	}
	...
	public void process_demand(Packet data) throws RdesktopException, IOException {
		...
		byte[] user = StringUtil.blankToDefault(creds == null ? null : creds.get(0), "").getBytes("US-ASCII");
		...
		try {
			if (!state.getOptions().isBuiltInLicence() && state.getOptions().isLoadLicence()) {
				...
				if ((licence_data != null) && (licence_data.length > 0)) {
					if (JulLog.isDebugEnabled())
						JulLog.debug("licence_data.length = " + licence_data.length);
					...
					if (JulLog.isDebugEnabled())
						JulLog.debug("Presented stored licence to server!");
					return;
				}
			}
	...
	public void process_issue(Packet data) throws IOException, RdesktopCryptoException {
		...
		try {
			 ...
			 */
			if (JulLog.isDebugEnabled())
				JulLog.debug("Server issued Licence");
	...
	public void send_request(byte[] client_random, byte[] rsa_data, byte[] username, byte[] hostname)
		...
		if (state.getOptions().isBuiltInLicence() && (!state.getOptions().isLoadLicence())
			...
			if (JulLog.isDebugEnabled())
				JulLog.debug("Using built-in Windows Licence");
			buffer.setLittleEndian32(0x03010000);
		} else {
			if (JulLog.isDebugEnabled())
				JulLog.debug("Requesting licence");
	...
	void save_licence(Packet data, int length) throws IOException {
		...
		for (int i = 0; i < 3; i++) {
			...
			if (data.getPosition() + 4 - startpos > length) {
				JulLog.warn("Error in parsing licence key.");
				return;
			}
		}
		len = data.getLittleEndian32();
		if (JulLog.isDebugEnabled())
			JulLog.debug("save_licence: len=" + len);
		 ...
		 * data.copyToByteArray(databytes,0,data.getPosition(),len);
		 * fd.write(databytes); fd.close(); JulLog.info("Stored licence at " +
		 * filepath); } catch(FileNotFoundException
		 * e){JulLog.info("save_licence: file path not valid!");}
		 * catch(IOException e){JulLog.warn("IOException in save_licence");}
		 */
	}
}
