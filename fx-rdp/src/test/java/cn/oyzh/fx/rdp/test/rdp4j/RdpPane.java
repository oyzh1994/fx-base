//package cn.oyzh.easyshell.test.rdp4j;
//
...
//            (scanCode, extended, release, shiftDown, shortcutModifierDown) ->
//                    frontend.forwardNativeKey(scanCode, extended, release, shiftDown,
...
//            uninstallNativeKeyboardCapture();
...
//            // requestFocus() does not emit another focused-property change when
//            // the RDP image already owns focus. Re-apply both pieces of native
//            // keyboard isolation so a restored/reparented window cannot retain
//            // the local Windows IME context or a stale hook state.
//            refreshNativeKeyboardCapture();
//            Platform.runLater(this::refreshNativeKeyboardCapture);
//        }
//    }
//
...
//    }
//
//    private void deactivateNativeKeyboardCapture() {
//        WindowsFullScreenKeyboardHook.instance().deactivate(fullScreenNativeKeySink);
//        windowsImeController.restore();
//    }
//
//    /**
//     * Applies keyboard capture from one authoritative focus predicate. The
//     * ImageView and its owning Window used to activate/deactivate the same
//     * singleton hook independently, so either listener could disable capture
//     * while the other focus owner was still valid.
//     */
//    private void refreshNativeKeyboardCapture() {
//        ImageView target = nativeKeyboardTarget;
//        Scene scene = target == null ? null : target.getScene();
//        Window window = scene == null ? null : scene.getWindow();
//        boolean focused = target != null && target.isFocused()
//                && window != null && window.isFocused();
//        if (!focused || !frontend.hasKeyboardInput()) {
//            deactivateNativeKeyboardCapture();
//            return;
//        }
//
//        // Detach the host IME before enabling the raw scan-code path. This
//        // keeps local Chinese/full-width state from consuming or translating
//        // keys that belong to the remote Windows input method.
//        windowsImeController.disableForFocusedWindow();
//        WindowsFullScreenKeyboardHook hook = WindowsFullScreenKeyboardHook.instance();
//        if (!hook.isActive(fullScreenNativeKeySink)) {
//            hook.activate(fullScreenNativeKeySink);
//        }
//    }
//
//    private void installNativeKeyboardCapture() {
//        uninstallNativeKeyboardCapture();
//        FxRdpDisplay display = frontend.getDisplay();
//        if (display == null) {
//            return;
//        }
//        ImageView target = display.getImageView();
//        ChangeListener<Boolean> listener = (observable, wasFocused, isFocused) ->
//                refreshNativeKeyboardCapture();
//        nativeKeyboardTarget = target;
//        nativeKeyboardFocusListener = listener;
//        target.focusedProperty().addListener(listener);
//        refreshNativeKeyboardCapture();
//    }
//
//    private void uninstallNativeKeyboardCapture() {
//        ImageView target = nativeKeyboardTarget;
//        ChangeListener<Boolean> listener = nativeKeyboardFocusListener;
//        nativeKeyboardTarget = null;
//        nativeKeyboardFocusListener = null;
